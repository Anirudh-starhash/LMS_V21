<template>
    <div :class="[isDarkMode?'dark':'main-class']">
        ...
        <div class="main-content">
          <h2 :class="[isDarkMode?'h2dark':'h2light']">{{this.get_info}}</h2>
          <h2 :class="[isDarkMode?'h2dark':'h2light']">Do you really want to Revoke the Book?</h2>
          <div class="buttons">
            <button @click="revokeBook" class="btn btn-success a">Yes</button>
            <button @click="cancel" class="btn btn-danger a">No</button>
           </div>
        </div>
        <footer_page/>
  ...
  export default {
     ...
     methods: {
       ...
       cancel(){
            this.$router.push({name:'request_page'});
        },
        async revokeBook(){
          try{
            const r=await axios.post("http://127.0.0.1:5000/api/revokeBook",
              JSON.stringify({
                "user_id":this.id,
      ...
      async mounted() {
        ...
        if(!access_token){
          ...
          try{
            ...
            if(r.status===200){
                const response=await axios.get(`http://127.0.0.1:5000/api/getinfoOfABook/${this.book_id}/${this.id}`);
                this.get_info=response.data.get_info
