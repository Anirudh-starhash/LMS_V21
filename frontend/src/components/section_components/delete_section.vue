<template>
    <div :class="[isDarkMode?'dark':'main-class']">
        ...
        <div class="main-content">
           <h2 :class="[isDarkMode?'h2dark':'h2light']">Do you really want to Delete the Section of ID {{this.section_id}}</h2>
           <div class="buttons">
  ...
  export default {
     ...
     methods: {
        ...
        back(){
            this.$router.push({name:"section_page"});
        },
        async deletebook(){
            try{
                const r=await axios.post("http://127.0.0.1:5000/api/deleteSection",
                    JSON.stringify({
