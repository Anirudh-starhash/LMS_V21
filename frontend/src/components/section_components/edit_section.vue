...
<template>
  ...
  export default {
   ...
   methods: {
    ...
    async editSection(){
      try{
        const response=await axios.post(`http://127.0.0.1:5000/api/editSection/${this.id}`,
          JSON.stringify({
            "title":this.title,
    ...
    async mounted(){
      ...
      if(!access_token){
        ...
        try{
          ...
          if(r.status===200){
            const response=await axios.get(`http://127.0.0.1:5000/api/getSection/${this.id}`)
            this.title=response.data.section_info["title"];
            this.date=response.data.section_info["date"]
            this.description=response.data.section_info["description"]
            this.img_url=response.data.section_info["img_url"]
          }
        }
        catch(error){
          console.log(error)
