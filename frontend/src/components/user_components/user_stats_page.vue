...
import Chart3d from './PieChart3d.vue';
import BarChart from '../library_components/BarChart.vue';
  ...
  async mounted() {
      ...
      if(!access_token){
        ...
        try{
          ...
          if(r.status===200){
            const response = await axios.get(`http://127.0.0.1:5000/api/getUserDetails/${this.id}`);
