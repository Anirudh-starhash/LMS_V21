<template>
    <div :class="[isDarkMode?'dark':'main-class']">
        ...
        <div class="main-content">
          ...
          <h2 :class="[isDarkMode?'h2dark':'h2light']">Do you really want to Return the Book?</h2>
          <div class="buttons">
            <button @click="returnBook" class="btn btn-success a">Yes</button>
  ...
  export default {
     ...
     methods: {
       ...
       cancel(){
        ...
        async returnBook(){
            try{
                const r=await axios.post("http://127.0.0.1:5000/api/returnBook",
                    JSON.stringify({
                        "user_id":this.id,
                ...
                if(r.status===200){
                    ...
                    this.$router.push({name:"reviews_page",params:{id:this.id}})
                }
                else{
                   alert('Some Error!')
                }
