      ...
      <div class="row">
        <form v-on:submit.prevent="editBook">
          ...
          <div class="mb-7">
            ...
            <p v-if="img_url" class="file-placeholder x3">Current file: {{ img_url }}</p>  
        </div>
          <div class="mb-7">
            ...
            <p v-if="pdf_link" class="file-placeholder x3">Current file: {{pdf_link }}</p>
          </div>
...
<script>
  ...
  export default {
    ...
    data(){
      return {
          message:"Welcome to Edit Book Page",
          isDarkMode:false,
          title:"",
          auth_lname:"",
          auth_fname:"",
    ...
    methods: {
      ...
      async editBook(){
        try{
          const response=await axios.post("http://127.0.0.1:5000/api/editBook",
            JSON.stringify({
              "id":this.id,
              "title":this.title,
    ...
    async mounted(){
      ...
      if(!access_token){
        ...
        this.id=localStorage.getItem("section_id")
        this.ISBN=localStorage.getItem("book_id")
        try{
          ...
          if(r.status===200){
              const response=await axios.get(`http://127.0.0.1:5000/api/getBook/${this.ISBN}`)
              this.title=response.data.book_info["title"];
              this.date=response.data.book_info["date"]
              this.no_of_pages=response.data.book_info["no_of_pages"]
              this.img_url=response.data.book_info["img_url"]
              this.auth_fname=response.data.book_info["auth_fname"]
              this.auth_lname=response.data.book_info["auth_lname"]
              this.publisher=response.data.book_info["publisher"]
              this.pdf_link=response.data.book_info["pdf_link"]
          }
        }
        catch(error){
            console.log(error)
