<template>
  ...
  export default {
   ...
   data(){
    return {
      message:'Welcome to Edit Review Page',
      review:"",
      email_error:"",
      isDarkMode: false,
      ratings:"",
   ...
   async mounted() {
      ...
      if(!access_token){
        ...
        try{
          ...
          if(r.status===200){
            const r1=await axios.get(`http://127.0.0.1:5000/api/getAReview/${this.id}/${this.book_id}`);
            this.review=r1.data.reviews["feedback"]
            this.ratings=r1.data.reviews["rating"]
    ...
    methods: {
      ...
      async submitreview(){
        try{
          ...
          if(r.status===200){
            this.$router.push({name:'read_reviews',params:{user_id:this.id,id:this.book_id}});
          }
          else{
            alert('Some error!')
          }
